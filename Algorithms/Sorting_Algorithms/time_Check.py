...
def merge_sort_t(A, first, last):
    if first >= last:
        ...
        return A
    mid = (first + last) // 2
    merge_sort_t(A, first, mid)
    merge_sort_t(A, mid + 1, last)
    i, j, B = first, mid + 1, []
    while i <= mid and j <= last:
        if A[i] <= A[j]:
            B.append(A[i])
    ...
    if A[i:mid + 1]: B.extend(A[i:mid + 1])
    if A[j:last + 1]: B.extend(A[j:last + 1])
...
time_counted = pd.DataFrame(columns = ['n', 'Qc', 'Qs', 'Qt', 'Mc', 'Ms', 'Mt', 'Hc', 'Hs', 'Ht'])
...
while count <= 500000:
    ...
    time_counted.loc[num, 'Qt'] = timeit.timeit("quick_sort_t(A, 0, len(A) - 1)", globals=globals(), number=1)
    print("time =", time_counted.loc[num, 'Qt'])
    Qc, Qs, Mc, Ms, Hc, Hs = 0, 0, 0, 0, 0, 0
    ...
    print("Merge sort:")
    time_counted.loc[num, 'Mt'] = timeit.timeit("merge_sort_t(B, 0, len(B) - 1)", globals=globals(), number=1)
    print("time =", time_counted.loc[num, "Mt"])
    Qc, Qs, Mc, Ms, Hc, Hs = 0, 0, 0, 0, 0, 0
    ...
    print("Heap sort:")
    time_counted.loc[num, 'Ht'] = timeit.timeit("heap_sort_t(C)", globals=globals(), number=1)
    print("time =", time_counted.loc[num, "Ht"])
    Qc, Qs, Mc, Ms, Hc, Hs = 0, 0, 0, 0, 0, 0
    C = heap_sort(C)
    print("comparisons = {:10d}, swaps = {:10d}\n".format(Hc, Hs))
